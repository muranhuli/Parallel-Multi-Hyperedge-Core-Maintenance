...
void insertEdge(vector<vector<int>> &hyperEdge, unordered_map<int, vector<int>> &hyperNode, unordered_map<int, int> &core, unordered_map<int, unordered_set<int>> &nodeinfo, vector<PII> &edgeinfo, const vector<vector<int>> &ee, int &vertex_num)
{
...
void dividDataSet(vector<vector<int>> &hyperEdge, vector<vector<int>> &InsertEdge, int n)
{
    ...
    for (int i = 0; i < n; i++)
    {
        InsertEdge.emplace_back(hyperEdge[int(hyperEdge.size() - 1)]);
        hyperEdge.pop_back();
    }
}
...
int main()
{
    ...
    while (ss >> str)
    {
        ...
        while (true)
        {
            ...
            {
                insertEdge(hyperEdge, hyperNode, core, nodeinfo, edgeinfo, it.second, vertex_num);
            }
        }
        auto t2 = std::chrono::steady_clock::now();
        double dr_ns1 = std::chrono::duration<double, std::nano>(t2 - t1).count();
        fout << dr_ns1 / num << " " << vertex_num / num << endl;
        cout << str << endl;
    }
    fout.close();
}