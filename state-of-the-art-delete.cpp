...
void eraseEdge(vector<vector<int>> &hyperEdge, unordered_map<int, vector<int>> &hyperNode, unordered_map<int, int> &core, int index, int &vertices_num)
{
    vector<int> e;
...
int main()
{
    ...
    while (ss >> str)
    {
        ...
        for (int i = 0; i < eraseEdgeSet.size(); i++)
        {
            eraseEdge(hyperEdge, hyperNode, core, eraseEdgeSet[i], vertices_num);
        }
        auto t6 = std::chrono::steady_clock::now();
        auto dr_ns3 = std::chrono::duration<double, std::nano>(t6 - t5).count();
        fout << str << " " << dr_ns3 / num << endl;
    }
    fout.close();
}